...
Trade::Trade(QObject *parent) : QObject(parent) {
    // Connect Websocket
    connect(&mWebsocket, QOverload<QAbstractSocket::SocketError>::of(&QWebSocket::error), this, &Trade::onError);
    connect(&mWebsocket, &QWebSocket::connected, [&]() { emit connectionChanged(true); });
    connect(&mWebsocket, &QWebSocket::disconnected, [&]() { emit connectionChanged(false); });
    connect(&mWebsocket, &QWebSocket::textMessageReceived, this, &Trade::onNewMessage);
}

Trade::~Trade() { closeLiveSearch(); }

bool Trade::isEnabled() const { return mEnabled; }

bool Trade::isConnected() const { return mWebsocket.state() == QAbstractSocket::ConnectedState; }

QString Trade::getId() const { return mId; }

QString Trade::getLeague() const { return mLeague; }

QString Trade::getName() const { return mName; }
...
void Trade::setId(QString id) { mId = id; }

void Trade::setLeague(QString league) { mLeague = league; }

void Trade::setName(QString name) { mName = name; }
...
void Trade::openLiveSearch() {
    if (isConnected()) { closeLiveSearch(); }

    QNetworkRequest connection(QStringLiteral("wss://www.pathofexile.com/api/trade/live/%1/%2").arg(mLeague).arg(mId));
...
void Trade::closeLiveSearch() { mWebsocket.close(); }
    ...
    data["query"] = QJsonObject{
        ...
        {"filters",
         QJsonObject{{"trade_filters",
                      QJsonObject{{"filters", QJsonObject{{"sale_type", QJsonObject{{"option", "priced"}}}}}}}}}};
...
void Trade::fetch(const QString &id, Callback &&slot) { fetch(QStringList{id}, std::forward<Callback>(slot)); }
...
void Trade::fetch(const QStringList &ids, Callback &&slot) {
    ...
    AccessManager::get(buildRequest(TradeFetchPath + idQuery), std::forward<Callback>(slot)); /*, &mRateLimiter);*/
}
...
void Trade::onNewMessage(const QString &message) {
    const auto &newItems = QJsonDocument::fromJson(message.toUtf8()).object().value("new").toArray();
...
void Trade::parseFetchedItems(const QByteArray &data) {
    ...
    for (const auto &raw_result : results) {
        const auto &&result     = raw_result.toObject();
        ...
        emit itemReceived({name + type, name, type, price["amount"].toInt(), price["currency"].toString()});
    }
}
