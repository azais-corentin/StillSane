...
double Ninja::getChaosEquivalent(const QString &tradeId) {
    if (mUpToDateCurrencyDetail) {
        auto it = std::find_if(mCurrencyNames.begin(), mCurrencyNames.end(),
                               [tradeId](const CurrencyDetail &detail) { return detail.tradeId == tradeId; });

        if (it != mCurrencyNames.end()) { return it->chaosEquivalent; }
    } else {
        if (!mUpdatingCurrencyDetail) fetchCurrencyOverview(currentLeague, "Currency");
    }
    return 0.0;
}
...
void Ninja::parseCurrencyOverview(const QByteArray &data) {
    ...
    for (const auto &raw_detail : currencyDetails) {
        const auto &&detail = raw_detail.toObject();
        mCurrencyNames.push_back({detail["name"].toString(), detail["tradeId"].toString(), detail["id"].toInt(), 0.0});
    }

    for (const auto &equivalent : currencyEquivalents) {
        const QString name = equivalent.toObject()["currencyTypeName"].toString();
        auto it            = std::find_if(mCurrencyNames.begin(), mCurrencyNames.end(),
                                          [name](const CurrencyDetail &detail) { return detail.name == name; });

        if (it != mCurrencyNames.end()) { it->chaosEquivalent = equivalent.toObject()["chaosEquivalent"].toDouble(); }
    }

    // Special case for Chaos Orb: set to 1.0 chaosEquivalent
    auto it = std::find_if(mCurrencyNames.begin(), mCurrencyNames.end(),
                           [](const CurrencyDetail &detail) { return detail.name == "Chaos Orb"; });
    if (it != mCurrencyNames.end()) { it->chaosEquivalent = 1.0; }
