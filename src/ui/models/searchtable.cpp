...
bool SearchTable::setData(const QModelIndex &index, const QVariant &value, int role) {
    ...
    case 0: {
        ...
        if (!name.isEmpty()) mTradeApis.at(index.row())->setName(name);
        nameChanged(id, name);
        break;
    }
    case 2:
...
QVariant SearchTable::data(const QModelIndex &index, int role) const {
    ...
    if (index.row() >= mTradeApis.size()) return {};
    ...
    if (role == Qt::DisplayRole) {
        ...
        case 2:
            return tradeApi->isEnabled() ? QStringLiteral("Enabled") : QStringLiteral("Disabled");
        }
        ...
        case 2:
            return (Qt::AlignVCenter | Qt::AlignHCenter).toInt();
        }
